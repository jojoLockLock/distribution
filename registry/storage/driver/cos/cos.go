...
func (w *writer) Commit() error {
	...
	fmt.Println(w.key, " - ", w.uploadID)

	fmt.Println(len(w.parts))
...
func (w *writer) flushPart() error {
	...
	if len(w.readyPart) != 0 {
		...
		fmt.Println("[INFO] flush:", chunkNumber)
...
func (d *driver) copy(ctx context.Context, sourcePath string, destPath string) error {
	fmt.Println("copy:", sourcePath, " - ", destPath)
	...
	if fileInfo.Size() <= multipartCopyThresholdSize {
		fmt.Println("[INFO] directly copy")
